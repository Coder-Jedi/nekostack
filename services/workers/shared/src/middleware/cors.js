...
import { createCORSResponse as createCORSResponseUtil } from '../utils/response';
...
export function createCORSResponse(origin, config) {
    const headers = {
        'Access-Control-Allow-Origin': config.origins.includes('*') ? '*' : origin || '*',
        'Access-Control-Allow-Methods': config.methods.join(', '),
        'Access-Control-Allow-Headers': config.headers.join(', '),
...
export function addCORSHeaders(response, context) {
...
export function createCORSFromEnv(env) {
