...
export function rateLimitMiddleware(options = {}) {
    ...
    return async (context) => {
        ...
        try {
            ...
            const currentCount = await kv.get(windowKey, false) || 0;
            ...
            if (currentCount >= config.maxRequests) {
                ...
                return {
                    success: false,
                    response: createRateLimitResponse(config.message, retryAfter, context.request_id)
                };
            }
            // Increment counter
            await kv.setWithExpiration(windowKey, (currentCount + 1).toString(), Math.ceil(config.windowMs / 1000));
...
export function apiKeyRateLimitMiddleware(options = {}) {
    const config = {
        ...DEFAULT_OPTIONS,
        keyGenerator: (context) => {
...
export function getRateLimitInfo(context) {
    return context.rateLimitInfo || null;
}
// Helper function to add rate limit headers to response
export function addRateLimitHeaders(response, context) {
...
export function createRateLimitKey(prefix, identifier, windowStart) {
    return `${prefix}:${identifier}:${windowStart}`;
}
// Helper function to check if request should be rate limited
export function shouldRateLimit(requestCount, maxRequests, skipSuccessful = false, skipFailed = false, isSuccess = true) {
    if (skipSuccessful && isSuccess)
        return false;
    if (skipFailed && !isSuccess)
        return false;
