// Middleware chain for API Gateway
...
const DEFAULT_OPTIONS: Required<MiddlewareOptions> = {
  ...
  authOptions: {
    required: false,
    skipPaths: ['/api/system/health', '/api/system/status']
  }
};
...
// Admin routes middleware (admin auth required)
export function createAdminMiddleware(): (context: RequestContext) => Promise<Response> {
  return createMiddlewareChain({
    enableCORS: true,
    enableRateLimit: true,
    enableAuth: true,
